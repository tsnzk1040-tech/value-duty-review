...
function summaryLengthBlock(flavor: SummaryFlavor): string {
  if (flavor === "light") {
    return [
      ...
      "- 文の区切りは「。」を使う。一文が終わったら「、」ではなく「。」。全部を「、」でつながない。",
      "- 文末は「〜してみた」「〜したい」「〜ていて」などでもよいが、です・ます・ね・だね・ですねで終わらない。",
...
export function buildSummaryInstructions(
  ...
  return [
    ...
    "- 文が二つ以上なら「。」で区切る。「、」だけで長くつながない（読点だらけにしない）。",
    "",
    ...
    "",
    "【良い例（分量・洗練の参考・中身は仮）】",
    "誠実という言葉を自分で調べて、正直さだけでなく思いやりや約束を守ることまで含めて捉え直していた。今日は関係者への返答を曖昧にせず期限どおり返すことを意識し、明日も同じ基準で返答してみたい",
    "",
    "【悪い例】",
    `${heading} の想い`,
...
export function buildSummaryReviseInstructions(
  ...
  return [
    ...
    "- 文の区切りは「。」を使う。「、」だけで長くつながない。",
    "- 文末は「〜してみた」「〜したい」「〜ていて」などでもよいが、です・ますで終わらない。",
    "",
    lens,
    "【いまの要約全文】",
    currentSummary.trim(),
...
function polishSummaryMid(mid: string): string {
  ...
  out = out.replace(/[、,]$/, "");
  out = out.replace(/という$/, "");
  // 文中の「。」は残す（全部「、」に潰さない）。定型枠の直前だけ末尾句点を外す
  out = out.replace(/[。．]+$/g, "");
  out = out.replace(/[、,]{2,}/g, "、");
  out = out.replace(/[。．]{2,}/g, "。");
...
function leaderFlavorBlock(flavor: LeaderFlavor): string {
  if (flavor === "angle") {
    return [
      ...
      "- 同テーマ前回があるときは必須で引用する。自然につながるときだけ今日へ接続し、無理なら紹介だけでよい。",
...
export function leaderRetrySuffix(requireSameThemeQuote = false): string {
  const quoteLine = requireSameThemeQuote
    ? "同テーマ前回は必須。本人コメントから、テーマに近い実践・試したこと（なければ調べたこと）を『』で出す。自然につながるときだけ『といっていて、』で今日へ接続。つながらないときは紹介だけ（例: 『…』と実践していましたね。）でよい。無理な因果つなぎ・テーマと無関係な紹介は不合格。『前回の〇〇さん』だけは禁止。"
    : "同テーマ前回が入力にあれば必須で引用。無ければ②は省略。『前回の〇〇さん』だけは前日と読めるので禁止。";
  return [
    "【再出力指示】",
    "直前の出力は不合格（短すぎ／お礼や要約定型の混入／リンク解説が主役／締めの強い誘い／布教・標語調／薄すぎ／要点メモ等のアプリ用語／同テーマ前回の欠落や無理な接続）。",
    "所感の型で再出力せよ: ①共感・感謝 ②同テーマ前回（引用＋自然接続、または紹介だけ） ③テーマに会話っぽく一言 ④チームへの『こうしたら？』1点。",
    quoteLine,
...
export function buildLeaderInstructions(
  ...
  const history = hasSameTheme
    ? [
        "【同テーマ前回（入力あり＝引用必須）】",
        "- 必ず本文に入れる。引用は **本人の振り返りコメント（投稿）** から。リーダー所感・要約の転用禁止。",
        "- 接続は二択:",
        "  A. 今日の投稿と意味が自然に重なるときだけ『といっていて、』で短くつなぐ",
        "  B. 重なりが薄い・無理があるときは **紹介だけ**（接続を作らない）。『』にはテーマに一番近い実践・試したこと（なければ調べたこと）を入れる。例: 同テーマ前回の〇〇さんは、『…』と実践していましたね。",
        "- 不合格: 名前だけ／中身のない『とも重なります』／つながらないのに因果を捏造する／長文コピペ／テーマと無関係な一文の紹介",
  ...
  return [
    ...
    hasSameTheme
      ? "② 同テーマ前回を必ず出す（省略禁止）。つながるなら短い接続、つながらないなら紹介だけ"
    ...
    "",
    "【良い例（中身は仮）】",
    "（接続できるとき）周囲に聞いて一次回答まで持っていったの、現場でも使えそうでいいですね。同テーマ前回の山田さんは、『期限を先に切ると曖昧な依頼が減る』といっていて、今日の聞き方の整理にも通じますね。検索して調べた結果も踏まえ、朝会で『誰に聞くか』を先に決める、に寄せてみたらどうでしょう。",
    "（紹介だけのとき）一次回答まで持っていったの、いいですね。同テーマ前回の山田さんは、『期限を先に切ってから動く』と試していましたね。今日みたいな曖昧な依頼を減らすなら、朝会で『誰に聞くか』を先に決める、をチームの型にしてみたらどうでしょう。",
    "",
    ...
    "同テーマ前回の山田さんは、『期限を先に切る』といっていて、今日の接客マナーにも同じです。（つながらないのに無理つなぎ）",
    "",
    history,
    buildCreedAlignmentBlock(input.themeLabel),
    "",
